    ...
    override fun bind(
        ...
        view.repeatWhenAttached {
            repeatOnLifecycle(Lifecycle.State.CREATED) {
                ...
                // State flow for clock position
                val clockPosition = MutableStateFlow(
                ...
                val clockSettingsObserver =
                    object : ContentObserver(Handler(Looper.getMainLooper())) {
                        override fun onChange(selfChange: Boolean, uri: Uri?) {
                            if (uri == statusBarClockUri) {
                                clockPosition.value = context.contentResolver.readClockPosition()
                            }
                        }
                    }
                ...
                if (StatusBarRootModernization.isEnabled) {
                    ...
                    launch {
                        combine(
                            ...
                            clockPosition,
                        ) { visibilityModel, hideForHun, position ->
                            Triple(visibilityModel, hideForHun, position)
                        }.collectLatest { (visibilityModel, hideForHun, position) ->

                            // We only want to hide left clock for HUN
                            val hunBlocksClock =
                                (position == CLOCK_POSITION_LEFT) && hideForHun
