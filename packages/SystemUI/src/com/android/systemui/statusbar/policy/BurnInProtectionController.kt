...
class BurnInProtectionController @Inject constructor(
    ...
    private var phoneStatusBarBurnInProtectionHandler: PhoneStatusBarBurnInProtectionHandler? = null
    ...
    private fun loadResources() {
        with(context.resources) {
            ...
            maxStatusBarOffsetY = minOf(
                SystemBarUtils.getStatusBarHeight(context) - getDimensionPixelSize(com.android.internal.R.dimen.status_bar_height_default),
                getDimensionPixelSize(R.dimen.status_bar_offset_max_y)
            ) / 2
        }
        calculateNavBarMaxOffset()
        logD {
            "statusBarStartOffsetsX = $statusBarStartOffsetsX, statusBarEndOffsetsX = $statusBarEndOffsetsX, maxStatusBarOffsetY = $maxStatusBarOffsetY"
        }
    }
    ...
    fun setPhoneStatusBarBurnInProtectionHandler(handler: PhoneStatusBarBurnInProtectionHandler?) {
        this.phoneStatusBarBurnInProtectionHandler = handler
    }
    ...
    private fun getBurnInOffset(offsetLimits: Pair<Int, Int>): Int {
        ...
        val period = amplitude * 2
        return Math.round(zigzag(shiftCounter.toFloat(), amplitude, period) + offsetLimits.first)
    }
    ...
    private fun updateViews(sbOffset: Pair<Offset, Offset>, nbOffset: Offset) {
        if (sbOffset != statusBarOffsets) {
            ...
            phoneStatusBarBurnInProtectionHandler?.offsetStatusBar(sbOffset.first, sbOffset.second)
